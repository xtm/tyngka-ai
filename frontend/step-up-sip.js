...
button.addEventListener("click", async () => {
    ...
    const stepUpPercent = Number(
        document.getElementById("step-up-percent").value
    );
    ...
    if (existingInvestment < 0) {
        alert("Existing investment cannot be negative.");
        return;
    }
    ...
    if (stepUpPercent < 0 || stepUpPercent > 100) {
        alert("Annual SIP increase must be between 0% and 100%.");
        return;
    }
